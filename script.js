...
function getRandomQuote() {
  ...
  random_qoutez = quotes[QuoteArray]
  return random_qoutez
}
...
***/
function printQuote() {
  const randomQuote = getRandomQuote(quotes); // get a random quote from the quotes array
  return (
    <div>
      <p className="quote">{randomQuote.quote}</p>
      <p className="source">
        {randomQuote.source}
        <span className="citation">{randomQuote.citation}</span>
        <span className="year">{randomQuote.year}</span>
      </p>
    </div>
  )
}
